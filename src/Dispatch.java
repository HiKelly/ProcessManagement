...
public class Dispatch {
    ...
    public static void run(){
        ...
        if(proc == null){   //running队列为空可直接加入到running队列
            Process readyProc = MyThread.delReadyProc();
            if(readyProc != null) {
            ...
            while (((proc.getIsBlocked() == false && now < proc.getTotalTime())
                    || (proc.getIsBlocked() == true && now < proc.getBlockedTime())
            ...
            else if(proc.getIsBlocked() == true && now >= proc.getBlockedTime()){   //进程被中断，加入blocked队列
                Wait.add(proc);
            }
